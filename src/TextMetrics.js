...
export default ({ children, fontSize, lineHeight = 1.2, use = 'hhea' }) => {
  ...
  let { capHeight, xHeight, ascent, descent, unitsPerEm, head } = font;
  ...
  const yMaxRatio = head['yMax'] / unitsPerEm;
  const yMinRatio = head['yMin'] / unitsPerEm;
  ...
  return (
    ...
    >
      ...
      <Guide.RulerH hue={120} y={yMaxOffset} label="yMax" />
      ...
      <Guide.RulerH hue={120} y={yMinOffset} label="yMin" />
    </div>
  );
};
